...
async function safeState(){return JSON.parse(await ev(`JSON.stringify((()=>{const v=e=>!!(e&&(e.offsetWidth||e.offsetHeight||e.getClientRects().length)),c=x=>String(x||'').replace(/\\s+/g,' ').trim().slice(0,120),ok=s=>/Sandbox|Production|unsaved|save|discard|leave|continue|confirm|cancel|close|apply|M\.M\.F Test/i.test(s);const dialogs=[...document.querySelectorAll('[role="dialog"],dialog')].filter(v).map(d=>({text:c(d.innerText).split(/\\n/).slice(0,4).join(' | '),buttons:[...d.querySelectorAll('button,[role="button"]')].filter(v).map(b=>c(b.innerText||b.textContent)).filter(ok).slice(0,20)}));const labels=[...document.querySelectorAll('*')].filter(v).map(e=>{const t=c(e.innerText||e.textContent),r=e.getBoundingClientRect();return{t,tag:e.tagName,y:Math.round(r.y),w:Math.round(r.width),h:Math.round(r.height)}}).filter(x=>x.t&&ok(x.t)&&x.t.length<80).slice(0,100);return{dialogs,labels}})())`)||'{}')}
...
try{
 ...
 console.log('SANDBOX_POINT='+JSON.stringify(await pointFor('Sandbox',true)));
 console.log('TOP_SANDBOX_CLICKED='+await mouseClick('Sandbox',true));await sleep(1200);
 ...
 console.log('NAMED_POINT='+JSON.stringify(await pointFor(BOX,false)));
 console.log('NAMED_CLICK='+await mouseClick(BOX,false));await sleep(1800);
