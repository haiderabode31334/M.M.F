...
try{
 ...
 const sw=await waitDomClick('Sandbox',true,50);console.log('SANDBOX_SWITCH='+sw);if(!sw)throw Error('SANDBOX_SWITCH_NOT_READY');await sleep(1800);
 let named=0;for(let i=0;i<40&&!named;i++){const t=await body();if(t.includes('You are editing '+BOX)){named=2;break}named=await domClick(BOX,false);if(named)break;const p=await clickDialogProceed();if(p){console.log('DIALOG_PROCEED='+JSON.stringify(p));await sleep(900)}else await sleep(350)}
