...
try{
  ...
  await new Promise((res,rej)=>{const t=setTimeout(()=>rej(new Error('WS_TIMEOUT')),10000);ws.onopen=()=>{clearTimeout(t);res();};ws.onerror=rej;});
  ...
  await nav('https://developers.tiktok.com/apps/',5000);
  const auth=String(await ev('location.href'));
  if(/\/login(?:$|[/?#])/i.test(auth)){console.log('AUTH=NOT_SAVED');process.exitCode=2;}
  else{
    console.log('AUTH=OK');
    const opened=await clickText(['^Connect an app$','^Create an app$','Connect an app','Create an app']);
    console.log('CREATE_FLOW_OPENED='+(opened?1:0));
    if(opened){
      await sleep(2500);
      const filled=await setName();
      console.log('APP_NAME_FILLED='+(filled?1:0));
      await sleep(500);
      ...
      console.log('CREATE_BUTTONS='+JSON.stringify(d.buttons||[]));
      console.log('CHECKBOX_COUNT='+(d.checkboxes||0));
      console.log('RADIO_COUNT='+(d.radios||0));
      console.log('SELECT_COUNT='+(d.selects||0));
      console.log('GENERIC_HINTS='+JSON.stringify(d.generic||[]));
    }
  }
