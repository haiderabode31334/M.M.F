  ...
  await nav('https://developers.tiktok.com/');if(/log in with your tiktok developer account/i.test(await body())){console.log('AUTH=NOT_SAVED');process.exitCode=2;}
    ...
    if(opened){
      ...
      if(owner.status!=='LEGAL_CONFIRMATION_REQUIRED'&&owner.status!=='OWNER_DECISION_REQUIRED'){
        let confirmed=await clickText(['^Confirm$','^Continue$','^Next$']);console.log('OWNER_CONFIRMED='+(confirmed?1:0));if(confirmed)await sleep(4500);
        const nameSet=await setAppName();console.log('APP_NAME_FILLED='+(nameSet?1:0));
        if(nameSet){const saved=await clickText(['^Save$','^Create$','^Connect$','^Continue$','^Next$','^Confirm$']);console.log('APP_NAME_SAVED='+(saved?1:0));if(saved)await sleep(5000);}
        const t=await body();console.log('TARGET_VISIBLE='+(t.includes(TARGET)?1:0));console.log('HAS_BASIC_INFO='+(/basic information|app details/i.test(t)?1:0));console.log('HAS_PRODUCTS='+(/products|add products/i.test(t)?1:0));console.log('HAS_SCOPES='+(/scopes/i.test(t)?1:0));console.log('DETAIL_PATH='+String(await ev('location.pathname')));
      }
    }
  }
