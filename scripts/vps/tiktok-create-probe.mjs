...
async function nav(url,ms=4500){await send('Page.navigate',{url});await sleep(ms);}
async function text(){return String(await ev('(document.body?.innerText||"").slice(0,16000)')||'');}
...
function portalish(t){return /manage apps|my apps|connect an app|create an app/i.test(t)||t.includes(TARGET);}
async function locateApps(){
  for(const u of ['https://developers.tiktok.com/apps/','https://developers.tiktok.com/manage-apps/','https://developers.tiktok.com/manage-apps','https://developers.tiktok.com/user/apps','https://developers.tiktok.com/portal/apps']){
    await nav(u);const t=await text();if(!/log in with your tiktok developer account/i.test(t)&&portalish(t))return true;
  }
  for(let attempt=0;attempt<14;attempt++){
    await nav('https://developers.tiktok.com/',3200);
    const count=await ev(`(()=>{const vis=e=>!!(e&&(e.offsetWidth||e.offsetHeight||e.getClientRects().length));const W=innerWidth;return [...document.querySelectorAll('header *,nav *')].filter(e=>{if(!vis(e))return false;const r=e.getBoundingClientRect();if(r.y>180||r.x<W*.45||r.width<8||r.height<8||r.width>180||r.height>140)return false;return e.matches('a,button,[role="button"],[tabindex]')||e.querySelector('img,svg');}).length;})()`);
    if(attempt>=Number(count||0))break;
    ...
    await sleep(900);const t=await text();if(/manage apps|my apps/i.test(t)){const ok=await clickText(['^Manage apps$','^My apps$','Manage apps','My apps']);if(ok){await sleep(4000);return true;}}
  }
  return false;
}
...
try{
  ...
  if(/log in with your tiktok developer account/i.test(await text())){console.log('AUTH=NOT_SAVED');process.exitCode=2;}
  else{
    console.log('AUTH=OK');const located=await locateApps();console.log('APPS_AREA_REACHED='+(located?1:0));
    if(located){
      const opened=await clickText(['^Connect an app$','^Create an app$','^Create app$','^Add app$','^New app$','Connect an app','Create an app']);console.log('CREATE_FLOW_OPENED='+(opened?1:0));
      if(opened){await sleep(2500);const filled=await setName();console.log('APP_NAME_FILLED='+(filled?1:0));await sleep(600);
        const raw=await ev(`JSON.stringify((()=>{const vis=e=>!!(e&&(e.offsetWidth||e.offsetHeight||e.getClientRects().length));const root=[...document.querySelectorAll('[role="dialog"],[aria-modal="true"]')].find(vis)||document.body;const btn=[...root.querySelectorAll('button,[role="button"],a')].filter(vis).map(e=>({t:(e.innerText||e.textContent||'').trim().replace(/\\s+/g,' ').slice(0,90),a:(e.getAttribute('aria-label')||'').slice(0,90),title:(e.getAttribute('title')||'').slice(0,90),disabled:!!(e.disabled||e.getAttribute('aria-disabled')==='true')})).filter(x=>/create|connect|confirm|continue|next|submit|save|done|cancel|back|close|accept|agree/i.test([x.t,x.a,x.title].join(' '))).slice(0,30);const inputs=[...root.querySelectorAll('input,textarea,select')].filter(vis);const generic=(root.innerText||'').split(/\\n+/).map(s=>s.trim()).filter(s=>s&&s.length<120&&/app owner|personal|individual|organization|app name|terms|agree|accept|required|create|connect|continue|confirm|submit/i.test(s)).slice(0,40);return {buttons:btn,checkboxes:inputs.filter(i=>i.type==='checkbox').length,radios:inputs.filter(i=>i.type==='radio').length,selects:inputs.filter(i=>i.tagName==='SELECT').length,generic};})())`);
